...
export default function GuestBrowsePage({ onSignIn }) {
  ...
  // Use production events if available, fall back to mock data
  const displayEvents = IS_PRODUCTION && (prodEvents || [])?.length > 0 ? prodEvents : (EVENTS || []);
  const filtered = filterType === "all" ? displayEvents : (displayEvents || []).filter(e => e?.type === filterType);
        ...
        {!selectedEvent && (
          <>
            ...
            <div style={{ display: "flex", gap: 14, marginBottom: 28 }}>
              {[
                { label: "Total Events", value: displayEvents.length, color: C.navy },
                { label: "Study Events", value: displayEvents.filter(e => e.type === "study").length, color: "#A0522D" },
                { label: "Lab / Process", value: displayEvents.filter(e => e.type === "lab").length, color: C.tealDark },
                { label: "Build Events", value: displayEvents.filter(e => e.type === "build").length, color: C.coral },
              ].map((s, i) => (
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
              {[
                { id: "all", label: "All Events", count: displayEvents.length },
                { id: "study", label: "Study", count: displayEvents.filter(e => e.type === "study").length },
                { id: "lab", label: "Lab / Process", count: displayEvents.filter(e => e.type === "lab").length },
                { id: "build", label: "Build", count: displayEvents.filter(e => e.type === "build").length },
              ].map(f => (
                ...
                return (
                  <div key={ev?.id} onClick={() => setSelectedEvent(ev)}
                    ...
                    <div style={{ display: "flex", gap: 12, fontSize: 12, color: C.gray400, marginBottom: 14 }}>
                      <span>Team of {ev?.teamSize || ev?.team_size || 2}</span>
                      <span>·</span>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 32 }}>
              ...
              <div>
                ...
                <p style={{ color: C.gray600, fontSize: 14 }}>
                  Team of {selectedEvent?.teamSize || selectedEvent?.team_size || 2} · {(selectedEvent?.topics || []).length} topics · Division B 2025–2026
                </p>
              </div>
            </div>
